    ...
    def pad_data_to_good_offset(
        ...
        for item in batch:
            ...
            for i in range(len(item["tokens"])):
                ...
                # Clipping logp range
                all_logprobs = [
                    max(min(lp, 0.0), -20.0) if lp != 1.0 else lp for lp in all_logprobs
                ]

                if token_masks:
                    shifted_masks = token_masks[1:]
                    all_advantages_padded = [
                        advantage if (m != -100) else 0.0 for m in shifted_masks
                    ]
                else:
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        messages_dict = [
            {k: v for k, v in msg.model_dump().items() if v is not None} for msg in request.messages
        ]
        ...
        # Load tool call parser if tools were provided
        tc_parser = None
        if request.tools:
            try:
                import sys
                ...
                tc_parser = get_parser(trainer.config.tinker.tool_call_parser)
            except Exception:
                pass
        ...
        for i, sequence in enumerate(result.sequences):
            ...
            # Parse tool calls out of raw text if tools were in the request
            tool_calls_out = None
            remaining_content = output_text
            if tc_parser and request.tools:
                try:
                    parse_result = tc_parser.parse(output_text)
                    if parse_result.tool_calls:
                        ...
                        remaining_content = parse_result.content
                except Exception:
                    pass
